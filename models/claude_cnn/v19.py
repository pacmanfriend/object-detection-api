...
# Пример использования с обучением
def main():
    """Пример использования ResNet-18 с обучением"""
    ...
    # Демонстрация обучения на случайных данных
    print("\nДемонстрация обучения...")

    # Создание случайных данных для демонстрации
    num_samples = 100
    train_data = np.random.randn(num_samples, 3, 224, 224).astype(np.float32)
    train_targets = np.random.randint(0, 1000, num_samples)

    # Создание загрузчика данных
    train_loader = DataLoader(train_data, train_targets, batch_size=8, shuffle=True)

    print(f"Создан загрузчик данных: {len(train_loader)} батчей")

    # Обучение на несколько эпох (для демонстрации)
    model.train(train_loader, num_epochs=2, lr=0.001)
...
def create_imagenet_dataloader(data_dir, batch_size=32, image_size=224):
    """
    Создание загрузчика данных для ImageNet

    Args:
        data_dir: путь к директории с данными ImageNet
    ...
    try:
        ...
        def load_imagenet_data(split='train'):
                ...
                for img_name in os.listdir(class_dir):
                    if img_name.lower().endswith(('.jpg', '.jpeg', '.png')):
                        ...
                        try:
                            ...
                            # Изменение формата с HWC на CHW
        ...
        train_images, train_labels = load_imagenet_data('train')

        print("Загрузка валидационных данных...")
        val_images, val_labels = load_imagenet_data('val')
        ...
        print(f"Обучающий набор: {len(train_images)} изображений")
        print(f"Валидационный набор: {len(val_images)} изображений")

        return train_loader, val_loader

    except ImportError:
        print("PIL не установлен. Используйте: pip install Pillow")
        return None, None
    except Exception as e:
...
def train_on_imagenet(data_dir, num_epochs=90, batch_size=32, lr=0.1):
    """
    Полное обучение ResNet-18 на ImageNet
    ...
    # Загрузка данных
    train_loader, val_loader = create_imagenet_dataloader(data_dir, batch_size)

    if train_loader is None:
        print("Не удалось загрузить данные")
        return

    # Создание критерия потерь и оптимизатора
    criterion = CrossEntropyLoss()

    # Learning rate schedule (уменьшение LR каждые 30 эпох)
    def get_lr(epoch):
        if epoch < 30:
            return lr
        elif epoch < 60:
    ...
    for epoch in range(num_epochs):
        ...
        optimizer = SGDOptimizer(model.get_parameters(), lr=current_lr, momentum=0.9, weight_decay=1e-4)

        print(f"\nEpoch {epoch + 1}/{num_epochs}, LR: {current_lr}")

        # Обучение
        model.train_epoch(train_loader, criterion, optimizer, epoch)
        ...
        if (epoch + 1) % 5 == 0:
            ...
            if accuracy > best_accuracy:
                ...
                print(f"Новая лучшая точность: {best_accuracy:.2f}%")
                # Сохранение модели (можно добавить save_model функцию)
...
def save_model(model, filepath):
    ...
    # Сохранение весов conv1
    weights['conv1.weight'] = model.conv1.weight.get()
    if model.conv1.bias is not None:
        weights['conv1.bias'] = model.conv1.bias.get()

    # Сохранение весов bn1
    weights['bn1.gamma'] = model.bn1.gamma.get()
    weights['bn1.beta'] = model.bn1.beta.get()
    weights['bn1.running_mean'] = model.bn1.running_mean.get()
    weights['bn1.running_var'] = model.bn1.running_var.get()

    # Сохранение весов слоев ResNet
    for layer_idx, layer in enumerate([model.layer1, model.layer2, model.layer3, model.layer4]):
        for block_idx, block in enumerate(layer):
            prefix = f'layer{layer_idx + 1}.{block_idx}'

            weights[f'{prefix}.conv1.weight'] = block.conv1.weight.get()
            if block.conv1.bias is not None:
                weights[f'{prefix}.conv1.bias'] = block.conv1.bias.get()

            weights[f'{prefix}.bn1.gamma'] = block.bn1.gamma.get()
            weights[f'{prefix}.bn1.beta'] = block.bn1.beta.get()
            weights[f'{prefix}.bn1.running_mean'] = block.bn1.running_mean.get()
            weights[f'{prefix}.bn1.running_var'] = block.bn1.running_var.get()

            weights[f'{prefix}.conv2.weight'] = block.conv2.weight.get()
            if block.conv2.bias is not None:
                weights[f'{prefix}.conv2.bias'] = block.conv2.bias.get()

            weights[f'{prefix}.bn2.gamma'] = block.bn2.gamma.get()
            weights[f'{prefix}.bn2.beta'] = block.bn2.beta.get()
            weights[f'{prefix}.bn2.running_mean'] = block.bn2.running_mean.get()
            weights[f'{prefix}.bn2.running_var'] = block.bn2.running_var.get()

            if block.downsample is not None:
                weights[f'{prefix}.downsample.conv.weight'] = block.downsample.conv.weight.get()
                if block.downsample.conv.bias is not None:
                    weights[f'{prefix}.downsample.conv.bias'] = block.downsample.conv.bias.get()
                weights[f'{prefix}.downsample.bn.gamma'] = block.downsample.bn.gamma.get()
                weights[f'{prefix}.downsample.bn.beta'] = block.downsample.bn.beta.get()
                weights[f'{prefix}.downsample.bn.running_mean'] = block.downsample.bn.running_mean.get()
                weights[f'{prefix}.downsample.bn.running_var'] = block.downsample.bn.running_var.get()

    # Сохранение весов fc
    weights['fc.weight'] = model.fc.weight.get()
    if model.fc.bias is not None:
        weights['fc.bias'] = model.fc.bias.get()
...
def load_model(model, filepath):
    ...
    # Загрузка весов conv1
    model.conv1.weight = gpuarray.to_gpu(weights['conv1.weight'])
    if 'conv1.bias' in weights:
        model.conv1.bias = gpuarray.to_gpu(weights['conv1.bias'])

    # Загрузка весов bn1
    model.bn1.gamma = gpuarray.to_gpu(weights['bn1.gamma'])
    model.bn1.beta = gpuarray.to_gpu(weights['bn1.beta'])
    model.bn1.running_mean = gpuarray.to_gpu(weights['bn1.running_mean'])
    model.bn1.running_var = gpuarray.to_gpu(weights['bn1.running_var'])

    # Загрузка весов слоев ResNet
    for layer_idx, layer in enumerate([model.layer1, model.layer2, model.layer3, model.layer4]):
        for block_idx, block in enumerate(layer):
            prefix = f'layer{layer_idx + 1}.{block_idx}'

            block.conv1.weight = gpuarray.to_gpu(weights[f'{prefix}.conv1.weight'])
            if f'{prefix}.conv1.bias' in weights:
                block.conv1.bias = gpuarray.to_gpu(weights[f'{prefix}.conv1.bias'])

            block.bn1.gamma = gpuarray.to_gpu(weights[f'{prefix}.bn1.gamma'])
            block.bn1.beta = gpuarray.to_gpu(weights[f'{prefix}.bn1.beta'])
            block.bn1.running_mean = gpuarray.to_gpu(weights[f'{prefix}.bn1.running_mean'])
            block.bn1.running_var = gpuarray.to_gpu(weights[f'{prefix}.bn1.running_var'])

            block.conv2.weight = gpuarray.to_gpu(weights[f'{prefix}.conv2.weight'])
            if f'{prefix}.conv2.bias' in weights:
                block.conv2.bias = gpuarray.to_gpu(weights[f'{prefix}.conv2.bias'])

            block.bn2.gamma = gpuarray.to_gpu(weights[f'{prefix}.bn2.gamma'])
            block.bn2.beta = gpuarray.to_gpu(weights[f'{prefix}.bn2.beta'])
            block.bn2.running_mean = gpuarray.to_gpu(weights[f'{prefix}.bn2.running_mean'])
            block.bn2.running_var = gpuarray.to_gpu(weights[f'{prefix}.bn2.running_var'])

            if block.downsample is not None and f'{prefix}.downsample.conv.weight' in weights:
                block.downsample.conv.weight = gpuarray.to_gpu(weights[f'{prefix}.downsample.conv.weight'])
                if f'{prefix}.downsample.conv.bias' in weights:
                    block.downsample.conv.bias = gpuarray.to_gpu(weights[f'{prefix}.downsample.conv.bias'])
                block.downsample.bn.gamma = gpuarray.to_gpu(weights[f'{prefix}.downsample.bn.gamma'])
                block.downsample.bn.beta = gpuarray.to_gpu(weights[f'{prefix}.downsample.bn.beta'])
                block.downsample.bn.running_mean = gpuarray.to_gpu(weights[f'{prefix}.downsample.bn.running_mean'])
                block.downsample.bn.running_var = gpuarray.to_gpu(weights[f'{prefix}.downsample.bn.running_var'])

    # Загрузка весов fc
    model.fc.weight = gpuarray.to_gpu(weights['fc.weight'])
    if 'fc.bias' in weights:
        model.fc.bias = gpuarray.to_gpu(weights['fc.bias'])

    print(f"Модель загружена из {filepath}")


# Дополнительная функция для обучения с продвинутыми возможностями
class ResNet18:
    # ... (предыдущий код остается без изменений) ...

    def train_epoch(self, train_loader, criterion, optimizer, epoch):
        """Обучение на одной эпохе с подробным логированием"""
        total_loss = 0.0
        correct = 0
        total = 0
        ...
        for batch_idx, (data, targets) in enumerate(train_loader):
            ...
            # Compute loss
            loss = criterion.forward(output, targets, self.kernels)
            avg_loss = gpuarray.sum(loss).get() / data.shape[0]

            # Backward pass
            grad_output = criterion.backward(self.kernels)
            self.backward(grad_output)

            # Optimization step
            optimizer.step(self.kernels)
            optimizer.zero_grad()

            # Statistics
            total_loss += avg_loss
            predictions = output.get()
            targets_cpu = targets.get()
            predicted = np.argmax(predictions, axis=1)
            correct += np.sum(predicted == targets_cpu)
            total += targets_cpu.shape[0]

            # Logging
            if batch_idx % 100 == 0:
                accuracy = 100.0 * correct / total
                print(f"  Batch {batch_idx}/{num_batches}, Loss: {avg_loss:.4f}, Acc: {accuracy:.2f}%")
...
if __name__ == "__main__":
    # Выбор режима: демонстрация или полное обучение
    mode = "demo"  # или "train_imagenet"

    if mode == "demo":
        main()
    elif mode == "train_imagenet":
        # Укажите путь к данным ImageNet
        imagenet_path = "/path/to/imagenet/data"
        train_on_imagenet(imagenet_path, num_epochs=90, batch_size=32, lr=0.1)
    else:
        print("Неизвестный режим. Используйте 'demo' или 'train_imagenet'")
    ...
    def backward(self, grad_output):
        """Обратный проход"""
        # Backward through fc
        grad = self.fc.backward(grad_output, self.kernels)

        # Reshape gradient for avgpool
        grad = grad.reshape(grad_output.shape[0], 512, 1, 1)

        # Note: AdaptiveAvgPool backward is complex, simplified here
        # grad = self.avgpool.backward(grad, self.kernels)

        # Backward through layer4
        for block in reversed(self.layer4):
            grad = block.backward(grad, self.kernels)
        ...
        grad = self.maxpool.backward(grad, self.kernels)
        grad = self.relu.backward(grad, self.kernels)
        grad = self.bn1.backward(grad, self.kernels)
        grad = self.conv1.backward(grad, self.kernels)

        return grad


    def get_parameters(self):
        """Возвращает все параметры модели для оптимизатора"""
        params = []

        # Collect all weights and biases
        params.extend([self.conv1.weight])
        if self.conv1.bias is not None:
            params.extend([self.conv1.bias])

        params.extend([self.bn1.gamma, self.bn1.beta])

        for layer in [self.layer1, self.layer2, self.layer3, self.layer4]:
            for block in layer:
                params.extend([block.conv1.weight])
                if block.conv1.bias is not None:
                    params.extend([block.conv1.bias])
                params.extend([block.bn1.gamma, block.bn1.beta])

                params.extend([block.conv2.weight])
                if block.conv2.bias is not None:
                    params.extend([block.conv2.bias])
                params.extend([block.bn2.gamma, block.bn2.beta])

                if block.downsample is not None:
                    params.extend([block.downsample.conv.weight])
                    if block.downsample.conv.bias is not None:
                        params.extend([block.downsample.conv.bias])
                    params.extend([block.downsample.bn.gamma, block.downsample.bn.beta])

        params.extend([self.fc.weight])
        if self.fc.bias is not None:
            params.extend([self.fc.bias])

        return params


    def train_step(self, data, targets, criterion, optimizer):
        """Один шаг обучения"""
        # Forward pass
        output = self.forward(data)
    ...
    def train(self, train_loader, num_epochs, lr=0.001, momentum=0.9, weight_decay=1e-4):
        """Обучение модели"""
        criterion = CrossEntropyLoss()
        optimizer = SGDOptimizer(self.get_parameters(), lr=lr, momentum=momentum, weight_decay=weight_decay)
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, (data, targets) in enumerate(train_loader):
                loss = self.train_step(data, targets, criterion, optimizer)
                total_loss += loss
                num_batches += 1
...
class Linear:
    ...
    def backward(self, grad_output, kernels):
        batch_size, input_size = self.input_cache.shape

        # Gradient w.r.t. input
        grad_input = gpuarray.empty_like(self.input_cache)

        total_inputs = batch_size * input_size
        block_size = 256
        grid_size = (total_inputs + block_size - 1) // block_size

        kernels['linear_backward_input'](
            grad_output, self.weight, grad_input,
            np.int32(batch_size), np.int32(input_size), np.int32(self.out_features),
            block=(block_size, 1, 1), grid=(grid_size, 1)
        )
        ...
        # Gradient w.r.t. weight
        total_weights = self.out_features * input_size
        grid_size = (total_weights + block_size - 1) // block_size

        kernels['linear_backward_weight'](
            self.input_cache, grad_output, self.grad_weight,
            np.int32(batch_size), np.int32(input_size), np.int32(self.out_features),
            block=(block_size, 1, 1), grid=(grid_size, 1)
        )
        ...
        # Gradient w.r.t. bias
        if self.bias is not None:
            grid_size = (self.out_features + block_size - 1) // block_size
            kernels['linear_backward_bias'](
                grad_output, self.grad_bias,
                np.int32(batch_size), np.int32(self.out_features),
                block=(block_size, 1, 1), grid=(grid_size, 1)
            )
...
class BasicBlock:
    ...
    def backward(self, grad_output, kernels):
        ...
        grad = self.relu.backward(grad_output, kernels)
        ...
        grad = self.bn2.backward(grad_main, kernels)
        grad = self.conv2.backward(grad, kernels)
        grad = self.relu.backward(grad, kernels)
        grad = self.bn1.backward(grad, kernels)
        grad_input = self.conv1.backward(grad, kernels)

        # Backward through downsample if exists
        if self.downsample is not None:
            grad_downsample = self.downsample.backward(grad_residual, kernels)
            # Add gradients
            size = grad_input.size
            block_size = 256
            grid_size = (size + block_size - 1) // block_size

            result = gpuarray.empty_like(grad_input)
            kernels['add'](
                grad_input, grad_downsample, result, np.int32(size),
                block=(block_size, 1, 1), grid=(grid_size, 1)
            )
            return result
        else:
            ...
            result = gpuarray.empty_like(grad_input)
            kernels['add'](
                grad_input, grad_residual, result, np.int32(size),
                block=(block_size, 1, 1), grid=(grid_size, 1)
            )
            return result
...
class Downsample:
    ...
    def backward(self, grad_output, kernels):
        grad = self.bn.backward(grad_output, kernels)
        grad = self.conv.backward(grad, kernels)
        return grad


class CrossEntropyLoss:
    """Cross Entropy Loss функция"""

    def __init__(self):
        self.logits_cache = None
        self.targets_cache = None

    def forward(self, logits, targets, kernels):
        self.logits_cache = logits
        self.targets_cache = targets

        batch_size = logits.shape[0]
        loss = gpuarray.empty((batch_size,), dtype=np.float32)

        block_size = 256
        grid_size = (batch_size + block_size - 1) // block_size

        kernels['cross_entropy_loss'](
            logits, targets, loss,
            np.int32(batch_size), np.int32(logits.shape[1]),
            block=(block_size, 1, 1), grid=(grid_size, 1)
        )

        return loss

    def backward(self, kernels):
        batch_size, num_classes = self.logits_cache.shape
        grad = gpuarray.empty_like(self.logits_cache)

        total_elements = batch_size * num_classes
        block_size = 256
        grid_size = (total_elements + block_size - 1) // block_size

        kernels['cross_entropy_grad'](
            self.logits_cache, self.targets_cache, grad,
...
class SGDOptimizer:
    ...
    def __init__(self, parameters, lr=0.01, momentum=0.9, weight_decay=1e-4):
        ...
        for i, param in enumerate(parameters):
            if self.momentum > 0:
                self.momentum_buffers[i] = gpuarray.zeros_like(param)
    ...
    def step(self, kernels):
        ...
        for i, param in enumerate(self.parameters):
            ...
            momentum_ptr = self.momentum_buffers[i].ptr if i in self.momentum_buffers else 0

            kernels['sgd_update'](
                param, grad, momentum_ptr,
                np.float32(self.lr), np.float32(self.momentum), np.float32(self.weight_decay),
                np.int32(size),
                block=(block_size, 1, 1), grid=(grid_size, 1)
            )
...
class DataLoader:
    """Простой загрузчик данных для обучения"""
    ...
    def __iter__(self):
        ...
        for start_idx in range(0, self.num_samples, self.batch_size):
            ...
            batch_data_gpu = gpuarray.to_gpu(batch_data.astype(np.float32))
            batch_targets_gpu = gpuarray.to_gpu(batch_targets.astype(np.int32))

            yield batch_data_gpu, batch_targets_gpu
...
class ResNet18:
    """ResNet-18 архитектура с обучением"""
